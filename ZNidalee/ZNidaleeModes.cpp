...
void ZNidaleeModes::OnGapCloser(const GapCloserSpell & args) {
    ...
    if (args.Source->GetTeam() != player->GetTeam()) {
        if (args.Source->IsValidTarget() && ex->Dist2D(args.Source) <= 200) {
            if (!ex->UnderEnemyTurret(player)) {
                if (ZNidalee::CatForm()) {
                    ZNidalee::Swipe(args.Source, "gap");
                    ZNidalee::SwitchForm(args.Source, "gap"); }
                else {
                    ZNidalee::CastSpear(args.Source, "gap");
                    ZNidalee::SwitchForm(args.Source, "gap"); } } } } }
